...
const { occupation } = require('../models/occupation1');
// => localhost:3000/adolescence/

router.get('/', async (req, res) => {
    try {
        const docs = await occupation.find().exec();
        res.json(docs);
    } catch (error) {
...
router.get('/:id', async (req, res) => {
    ...
    try {
        const docs = await occupation.findById(req.params.id).exec();
        res.send(docs);
    } catch (err) {
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const updatedDocument = await occupation.findByIdAndUpdate(
            req.params.id,
            { $set: ad },
            { new: true }
        ).exec();
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const deletedDetails = await occupation.findByIdAndDelete(req.params.id).exec();
