...
router.get('/checkRegistrationNo/:registrationNo', async (req, res) => {
    const registrationNoToCheck = req.params.registrationNo;

    try {
        const patient = await Patients.findOne({ registrationno: registrationNoToCheck });
        ...
        console.error('Error in checking Registration No: ' + JSON.stringify(error, undefined, 2));
